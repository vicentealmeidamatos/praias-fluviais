...
const server = createServer(async (req, res) => {
  ...
  // ─── Save Site Content (desenvolvimento local) ───
  if ((req.method === 'POST' || req.method === 'GET') && url === '/api/save-content') {
    try {
      // Carregar .env
      ...
      const handler = (await import('./api/save-content.js')).default;
      let body = null;
      if (req.method === 'POST') {
        const chunks = [];
        for await (const chunk of req) chunks.push(chunk);
      ...
      const fakeReq = { method: req.method, body, headers: req.headers, [Symbol.asyncIterator]: function* () {} };
      const fakeRes = {
      ...
      console.error('[/api/save-content] Erro:', err.message);
      res.writeHead(500, { 'Content-Type': 'application/json', ...CORS });
      res.end(JSON.stringify({ error: err.message }));
    }
    return;
